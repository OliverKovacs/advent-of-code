...
const dijkstra = i => {
    ...
    add({ y: 4, x: 0, d: 1, n: 4, l: 13 });
    add({ y: 0, x: 4, d: 3, n: 4, l: 13 });
    ...
    while (queue.size() > 0) {
        ...
        A
            .filter(e => top.d !== R[e])
            .filter(d => !(d === top.d && top.n === 10))
            // .filter(d => (d === top.d && top.n < 3) || top.n >= 3)
            .map(d => [ d, D[d], (top.d !== d) ? 4 : 1 ])
            .map(([ d, delta, s ]) => [ d, [ top.y + s * delta[0], top.x + s * delta[1] ], s ])
            ...
            .forEach(([ d, [ y, x ], s]) => add({
                ...
                l: top.l + Array(s)
                    .fill(0)
                    .map((_, e) => i[y + e * D[R[d]][0]][x + e * D[R[d]][1]])
                    .reduce((p, c) => p + c),
            }));
    }
